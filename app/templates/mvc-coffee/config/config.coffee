...
config =
  ...
  production:
    ...
    db: 'mysql://localhost/<%= slugify(appname) %>-production'<% } %><% if(options.database == 'postgresql'){ %>
    db: 'postgres://localhost/<%= slugify(appname) %>-production'<% } %><% if(options.database == 'sqlite'){ %>
    db: 'sqlite://localhost/<%= slugify(appname) %>-production'
    storage: rootPath + '/data/<%= slugify(appname) %>-production'<% } %><% if(options.database == 'rethinkdb'){ %>
